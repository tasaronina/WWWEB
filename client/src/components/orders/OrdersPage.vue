<template>
  <v-container class="py-6">
    <div class="d-flex align-center mb-3 ga-3">
      <div class="text-h5">Заказы (админ)</div>
      <v-btn variant="outlined" size="small" :loading="loading" @click="loadAll">Обновить</v-btn>
    </div>

    <v-card class="mb-3">
      <v-card-text>
        <v-row class="align-center">
          <v-col cols="12" sm="4"><v-text-field v-model="filters.id" label="Фильтр по ID" /></v-col>
          <v-col cols="12" sm="4"><v-text-field v-model="filters.customer" label="Фильтр по клиенту" /></v-col>
          <v-col cols="12" sm="4">
            <v-select v-model="filters.status" :items="statusOptions" label="Статус" clearable />
          </v-col>
        </v-row>
        <div class="mt-2 d-flex ga-2">
          <v-btn variant="outlined" @click="resetFilters">Сброс</v-btn>
        </div>
      </v-card-text>
    </v-card>
    ...
    <v-data-table
      ...
      item-key="id"
    >
      <template #item.customer="{ item }">{{ customerName(item) }}</template>
      <template #item.items="{ item }">
        ...
        <span v-else class="text-medium-emphasis">Пусто</span>
      </template>
      <template #item.total="{ item }"><strong>{{ price(total(item.id)) }}</strong></template>
      <template #item.created_at="{ item }">{{ humanDate(item.created_at || item.created) }}</template>
      ...
      <template #no-data><div class="py-6 text-medium-emphasis">Заказов нет</div></template>
    </v-data-table>
        ...
        <v-card-text>
          <v-row class="mb-2">
            ...
            <v-col cols="12" md="6" class="d-flex align-end justify-end">
              <div class="text-medium-emphasis">Итого: <strong>{{ price(editSum) }}</strong></div>
            </v-col>
          </v-row>
          ...
          <v-table density="comfortable">
            ...
            <tbody>
              ...
              <tr v-if="!editing.rows.length">
                <td colspan="6" class="text-center text-medium-emphasis py-4">Пусто</td>
              </tr>
            </tbody>
          </v-table>
...
function customerName(o){
  return o?.customer?.name || o?.customer_name || o?.user?.username || o?.user || '—'
}
...
async function loadAll(){
  ...
  try{
    ...
    orders.value = Array.isArray(ordersResp) ? ordersResp.sort((a,b)=>a.id-b.id) : []
    const grouped = {}
...
const filteredOrders = computed(()=>{
  ...
  const q = filters.value.customer.trim().toLowerCase()
  const st = String(filters.value.status||'').trim()
  return orders.value.filter(o=>{
    if(id && !String(o.id).includes(id)) return false
    if(q && !customerName(o).toLowerCase().includes(q)) return false
    if(st && String(o.status||'') !== st) return false
    return true
  })
})
...
const editSum = computed(()=> editing.rows.reduce((s,r)=> s + r.price * r.qty, 0))
